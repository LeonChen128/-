...
foreach ($sql_purchase->fetchAll() as $row_purchase) {
  ...
  echo '<table class="record"><tr style="background-color:rgb(243, 173, 220);"><th>商品編號</th><th>商品名稱</th><th>價格</th><th>數量</th><th>小計</th></tr>';
  $total = 0;
  foreach ($sql_detail->fetchAll() as $row_detail) {
    echo '<tr><td>' . $row_detail['id'] . '</td>';
    echo '<td>' . $row_detail['name'] . '</td>';
    echo '<td>' . $row_detail['price'] . '</td>';
    echo '<td>' . $row_detail['count'] . '</td>';
    echo '<td>' . $row_detail['price'] * $row_detail['count'] . '</td>';
    $subtotal = $row_detail['price'] * $row_detail['count'];
    $total += $subtotal;
  }
  echo '<tr><td>總計</td><td></td><td></td><td></td><td>' . $total . '</td></tr>';
  echo '</table>';
}
